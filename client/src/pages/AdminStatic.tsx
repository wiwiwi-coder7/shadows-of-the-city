...
function Overview({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  const [error, setError] = useState("");
  const [choiceChapter, setChoiceChapter] = useState<number | null>(null);
  const [refreshing, setRefreshing] = useState(false);
  const load = useCallback(async () => {
    try { setError(""); setRefreshing(true); setData(await getOwnerDashboard(choiceChapter)); } catch (cause) { setError(errorText(cause)); if (cause instanceof OwnerApiError && cause.status === 401) onUnauthorized(); } finally { setRefreshing(false); }
  }, [choiceChapter, onUnauthorized]);
  useEffect(() => { void load(); }, [load]);
  if (!data && !error) return <div className="admin-skeleton">Reading the city’s signals…</div>;
  return <div className="admin-view">{error && <div className="form-error">{error}</div>}<section className="metric-grid"><Metric icon={UsersRound} label="Anonymous installations" value={data?.installations ?? 0} /><Metric icon={Activity} label="Game starts" value={data?.starts ?? 0} /><Metric icon={CheckCircle2} label="Completed cases" value={data?.completions ?? 0} /><Metric icon={FilePenLine} label="Recorded events" value={data?.events ?? 0} /></section><section className="admin-grid-two"><article className="admin-card"><div className="card-heading"><h2>Chapter reach</h2><span>Unique installations</span></div><div className="bar-chart">{data?.chapterReach.map(point => <div key={point.chapter}><span>CH {String(point.chapter).padStart(2, "0")}</span><i><b style={{ width: `${Math.min(100, (point.reached / Math.max(1, data.installations)) * 100)}%` }} /></i><em>{point.reached}</em></div>)}</div></article><article className="admin-card"><div className="card-heading"><h2>Choice coverage</h2><span>{choiceChapter ? `Chapter ${choiceChapter}` : "All chapters"}</span></div><div className="choice-summary"><b>{data?.choiceEvents ?? 0}</b><span>choice selections</span><b>{data?.choicePaths ?? 0}</b><span>distinct options traced</span></div><button className="button-secondary" onClick={() => void load()} disabled={refreshing}>{refreshing ? "Refreshing…" : "Refresh metrics"}</button></article></section>{data && <ChoiceAnalytics data={data} choiceChapter={choiceChapter} onChapterChange={setChoiceChapter} refreshing={refreshing} />}<section className="privacy-note"><ShieldCheck size={17} /><p><strong>Privacy boundary:</strong> these charts aggregate anonymous installation signals only. No save file, identity, or personal information is retained.</p></section></div>;
}
...
function ChoiceAnalytics({ data, choiceChapter, onChapterChange, refreshing }: { data: OwnerDashboard; choiceChapter: number | null; onChapterChange: (chapter: number | null) => void; refreshing: boolean }) {
  return <section className="choice-analytics"><div className="card-heading choice-analytics__heading"><div><h2>Choice analysis</h2><span>{choiceChapter ? `Chapter ${choiceChapter} · ranked volume and option split` : "Ranked volume and option split by narrative node"}</span></div><label className="chapter-filter">Chapter<select aria-label="Filter choice analytics by chapter" value={choiceChapter ?? "all"} onChange={event => onChapterChange(event.target.value === "all" ? null : Number(event.target.value))}>{["all", ...Array.from({ length: 10 }, (_, index) => String(index + 1))].map(value => <option value={value} key={value}>{value === "all" ? "All chapters" : `Chapter ${value}`}</option>)}</select></label></div>{refreshing && <p className="analytics-refreshing">Updating choice analysis…</p>}<section className="admin-grid-two"><article className="admin-card"><div className="card-heading"><h2>Most selected paths</h2><span>Top 24 options</span></div>{data.choices.length ? <ol className="choice-ranking">{data.choices.map((choice, index) => <li key={choice.id}><span className="choice-ranking__rank">{String(index + 1).padStart(2, "0")}</span><div><b>{choice.id}</b><small>Chapter {choice.chapter ?? "—"} · {choice.uniqueInstallations} unique installation{choice.uniqueInstallations === 1 ? "" : "s"}</small></div><strong>{choice.selections}</strong></li>)}</ol> : <EmptyCopy text="Choice signals will appear when players make their first decision." />}</article><article className="admin-card"><div className="card-heading"><h2>Decision splits</h2><span>Share within each node</span></div>{data.choiceNodes.length ? <div className="choice-splits">{data.choiceNodes.map(node => <article key={node.nodeId}><header><div><b>{node.nodeId}</b><small>Chapter {node.chapter ?? "—"} · {node.uniqueInstallations} unique installation{node.uniqueInstallations === 1 ? "" : "s"}</small></div><em>{node.selections} signals</em></header>{node.options.map(option => <div className="choice-split" key={option.option}><span>Option {option.option}</span><i><b style={{ width: `${option.percent}%` }} /></i><strong>{option.percent}%</strong><small>{option.selections}</small></div>)}</article>)}</div> : <EmptyCopy text="Choice signals will appear when players make their first decision." />}</article></section></section>;
}
