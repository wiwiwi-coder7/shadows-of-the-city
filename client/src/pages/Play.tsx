...
export default function Play() {
  ...
  const node: StoryNode | undefined = useMemo(() => save ? runtimeNodeById[save.currentNodeId] : undefined, [save, runtimeNodeById]);
  const trackEvent = (eventType: "game_start" | "node_view" | "choice_selected" | "chapter_reached" | "game_complete", choiceId?: string) => {
    ...
    track.mutate({ installationId: getInstallationId(), eventType, chapter: node.chapter, nodeId: node.id, choiceId: choiceId ?? null, locale: "en" });
  };
  ...
  return <main className={playerClass} onKeyDown={onStoryKeyDown}>
    ...
    <button className="screen-toggle" onClick={() => setShowHud(!showHud)} aria-label="Toggle controls"><Pause size={12} /></button>
