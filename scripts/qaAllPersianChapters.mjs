...
async function loadChapter(sample) {
  ...
  const before = JSON.parse(await evaluate("JSON.stringify({ language: document.documentElement.lang, direction: document.documentElement.dir, hasPersian: /[\u0600-\u06FF]/.test(document.querySelector('.story-copy')?.innerText ?? ''), hasFallback: Boolean(document.querySelector('.story-fallback')), choices: document.querySelectorAll('.choice-list button').length, node: localStorage.getItem('shadows-of-the-city:save') })"));
  ...
  await pause(500);
  const after = JSON.parse(await evaluate("JSON.stringify({ hasPersian: /[\u0600-\u06FF]/.test(document.querySelector('.story-copy')?.innerText ?? ''), hasFallback: Boolean(document.querySelector('.story-fallback')), node: JSON.parse(localStorage.getItem('shadows-of-the-city:save')).currentNodeId })"));
