...
- [ ] Audit the current browser-local save and Continue flow, then complete and verify a player-facing progress-save experience that resumes exactly at the last story node with its choices, flags, unlocks, and settings.
- [ ] Replace the GitHub Pages redirect with a directly playable static build that does not depend on Manus hosting, while preserving browser-local save and clearly scoping any server-only admin, telemetry, and live-editor functions.
- [ ] Diagnose and eliminate the GitHub Pages 404 path by adding a repository-root deployment configuration and verifying the public Pages URL after the static game build is published.
- [ ] Preserve owner-only admin editing, immediate Persian overrides, and anonymous telemetry through Supabase while removing Manus hosting from the player-facing GitHub Pages build.
- [ ] Keep the current identifier-and-password owner login by moving verification and signed admin-session issuance to protected Supabase endpoints, without exposing password hashes or elevated keys in the public GitHub Pages build.
